...
import moviepy
# from moviepy.video.fx.all import crop as crop_func
    ...
    while tot_dur < max_duration:
        for video_path in video_paths:
            ...
            if round((clip.w/clip.h), 4) < 0.5625:
                clip = moviepy.video.fx.all.crop(clip, width=clip.w, height=round(clip.w/0.5625), \
                            x_center=clip.w / 2, \
                            y_center=clip.h / 2)
            else:
                clip = moviepy.video.fx.all.crop(clip, width=round(0.5625*clip.h), height=clip.h, \
                            x_center=clip.w / 2, \
                            y_center=clip.h / 2)
...
def generate_video(combined_video_path: str, tts_path: str, subtitles_path: str, threads: int, subtitles_position: str,  text_color : str) -> str:
    ...
    print("error break here: *********************** text clip")
    ...
    print("error end here: *********************** text clip")
